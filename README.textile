...
And DynamicSifter will returns our results just like any <pre>ActiveRecord::#find</pre> query!
...
Let's say we store our publisher contact information in a <pre>Publisher</pre> model.  If we wanted to find the all the publishers for the books by Yukio Mishima that are paperback, in-stock, and have the list of publishers by in alphabetical order by name, we could do:
